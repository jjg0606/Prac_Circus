...
void CircusScene::MakeObjects()
{
	...
	RandomGenerateFlamePot(PotStartPoint , PotEndPoint - 2 * bitmapInfo[IMGEnum::FRONT].bmWidth, 600, 1200);
	RandomGenerateFlameRing(StartPos + windowRect.right, 55, 800, 1600);
...
void CircusScene::RevivePlayer()
{
	...
	objectVec.push_back(playerInstance);
	StartObjects();
...
void CircusScene::Regenerate(int playerX)
{
	...
	int PotEndPoint = EndPos - bitmapInfo[IMGEnum::FRONT].bmWidth;
	GenerateFlamePot(PotEndPoint);
	int PotStartPoint = playerX + 10 * bitmapInfo[IMGEnum::FRONT].bmWidth;
	RandomGenerateFlamePot(PotStartPoint, PotEndPoint - 2 * bitmapInfo[IMGEnum::FRONT].bmWidth, 600, 1200);
	RandomGenerateFlameRing(playerX + windowRect.right, 55, 800, 1600);

	EndTrampoline* endTramp = new EndTrampoline(hBitmap, bitmapInfo, windowRect, 3, EndPos, windowRect.bottom - bitmapInfo[IMGEnum::MITER].bmHeight - bitmapInfo[IMGEnum::END].bmHeight);
	objectVec.push_back(endTramp);
	colVec.push_back(endTramp);
