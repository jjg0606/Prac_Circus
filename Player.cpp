...
void Player::Update()
{
	if (Vpos.y == groundY && cntMiddleObs > 0)
	{
		score += cntMiddleObs * cntMiddleObs * 100;
		cntMiddleObs = 0;
	}
	SetState();
	...
	if (Vpos.x < 0) // cannot go out of rect
	{
		Vpos.x = 0;
		SetColider();
	}
...
void Player::Render(HDC hdc, int CameraX)
{
	...
	{
		cntDieAnim++;
		if (cntDieAnim > DieAnimPlayTime && Life >= 0)
		{ 
			cntDieAnim = 0;
